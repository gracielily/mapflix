...
describe("My Movies Behaviour", () => {
    beforeEach(() => {
        // cy.intercept("/authenticate", {statusCode: 302}).as("authenticate");
        cy.visit(`${baseUrl}/login`);
        cy.get("input").first().type("jane@doe.com");
        cy.get("input").last().type("Passw0rd?123");
    ...
    it("displays error message when movie cannot be created", () => {
        ...
        cy.get(".message-body").should("exist").and("contain.text", "\"imdbId\" with value \"invalidId\" fails to match the required pattern")
