...
suite("User API Tests", () => {
  setup(async () => {
    ...
    for (let i = 0; i < testUsers.length; i += 1) {
      ...
      testUsers[i] = await mapflixService.createUser(testUsers[i]);
    }
  });
  teardown(async () => {
  });
  ...
  test("Gets a User - Success", async () => {
    const returnedUser = await mapflixService.getUser(testUsers[0]._id);
  ...
  test("Gets a User - User Deleted", async () => {
    await mapflixService.deleteAllUsers();
    try {
      await mapflixService.getUser(testUsers[0]._id);
      assert.fail("Should not return a response");
    } catch (error) {
      assert(error.response.data.message === "No User with this id");
